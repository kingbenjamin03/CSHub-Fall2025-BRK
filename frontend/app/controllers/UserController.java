...
public class UserController extends Controller {
    ...
    public Result handleTestPagePost() {
        ...
        try {
            String name = loginForm.field("name").value();
            ...
            JsonNode response = RESTfulCalls.postAPI(RESTfulCalls.getBackendAPIUrl(config,
                    "/user/testpage"), jsonData);

            if (response.has("error")) {
                flash("error", "Error submitting form data");
                return badRequest(testPage.render("Error submitting form."));
            }

        } catch (Exception e) {
            Logger.debug("Frontend UserController handleTestPagePost() exception: " + e.toString());
            return badRequest(testPage.render("An error occurred."));
        }

        flash("success", "Form data sent to backend successfully!");
        return ok(testPage.render("Success!"));
    }
